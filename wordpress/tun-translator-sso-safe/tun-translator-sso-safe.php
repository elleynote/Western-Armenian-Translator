<?php
/**
 * Plugin Name: Tun Translator SSO Bridge
 * Description: TunApp checkout identity and first-party Translator SSO bridge.
 * Version: 2.0.7
 * Author: Tun
 */

if ( ! defined( 'ABSPATH' ) ) {
    exit;
}

const TUN_TRANSLATOR_SSO_SAFE_VERSION = '2.0.7';
const TUN_TRANSLATOR_SSO_INSTALL_OPTION = 'tun_translator_sso_install_v207';
const TUN_TRANSLATOR_SSO_INSTALL_ERROR_OPTION = 'tun_translator_sso_install_error_v207';

/*
 * The current production package already contains these files.
 * Load them directly instead of using is_readable(), which can return false
 * on some shared-hosting permission setups even when WordPress can access the files.
 */
require_once __DIR__ . '/internal/core/includes/class-tun-sso-settings.php';
require_once __DIR__ . '/internal/core/includes/class-tun-sso-provider.php';
require_once __DIR__ . '/internal/core/tun-saas-core.inc';

function tun_translator_sso_safe_maybe_install() {
    if ( ! current_user_can( 'manage_options' ) ) {
        return;
    }

    if ( get_option( TUN_TRANSLATOR_SSO_INSTALL_OPTION ) === TUN_TRANSLATOR_SSO_SAFE_VERSION ) {
        return;
    }

    if ( ! class_exists( 'Tun_SSO_Provider', false ) ) {
        update_option(
            TUN_TRANSLATOR_SSO_INSTALL_ERROR_OPTION,
            'The Tun SSO provider class could not be loaded.',
            false
        );
        return;
    }

    try {
        Tun_SSO_Provider::activate();

        update_option(
            TUN_TRANSLATOR_SSO_INSTALL_OPTION,
            TUN_TRANSLATOR_SSO_SAFE_VERSION,
            false
        );

        delete_option( TUN_TRANSLATOR_SSO_INSTALL_ERROR_OPTION );
    } catch ( Throwable $error ) {
        update_option(
            TUN_TRANSLATOR_SSO_INSTALL_ERROR_OPTION,
            'Tun SSO database setup could not complete: ' . sanitize_text_field( $error->getMessage() ),
            false
        );
    }
}
add_action( 'admin_init', 'tun_translator_sso_safe_maybe_install', 50 );

function tun_translator_sso_safe_admin_notice() {
    if ( ! current_user_can( 'manage_options' ) ) {
        return;
    }

    $message = get_option( TUN_TRANSLATOR_SSO_INSTALL_ERROR_OPTION );

    if ( ! is_string( $message ) || '' === trim( $message ) ) {
        return;
    }

    echo '<div class="notice notice-error"><p><strong>Tun Translator SSO:</strong> ' . esc_html( $message ) . '</p></div>';
}
add_action( 'admin_notices', 'tun_translator_sso_safe_admin_notice' );
